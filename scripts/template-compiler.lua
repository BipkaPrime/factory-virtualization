-- This file contains is used to compile production and research templates.
...
-- Enables/disables all labs on a a given surface.
    ...
    for _, entity in ipairs(entities) do
        ...
        if entity and entity.valid and entity.type ~= "entity-ghost" then
            ...
            if build_cost then
                ...
                add_to_cost(total_cost, build_cost[1].name, build_cost[1].count, quality)
...
function Helper.can_start_compilation(surface_id, template_name)
    if not utils.template_name_available(template_name) then
        game.print("Provided template name unavailable")
        return false
    end
    -- cheking if given surface is a vsurface
    local vsurface_data = storage.v_surfaces[surface_id]
    if not vsurface_data then return false end
    -- checking if given surface is already compiling
    if storage.compiling_surfaces[surface_id] ~= nil then return false end
    -- checking that surface is valid
    local surface = game.get_surface(surface_id)
    if not surface and surface.valid then return false end
    -- checking if science template compilation is already ongoing
    local surface_type = vsurface_data.type
    -- type 2 is science surface
    if surface_type == 2 then
        for _, venv in pairs(storage.compiling_surfaces) do
            if venv.surface_type == 2 then
                game.print("Another science template is already compiling")
                return false
            end
        end
    end
...
local function create_production_template(venv, surface, template_name)
    ...
    if venv.surface_type == 2 then
        ...
        for ingredient, data in pairs(venv.benchmark_results.slow) do
            ...
            -- gathering cumulative stats for all science packs used in benchmarks
            science_packs[ingredient] = science_packs[ingredient] or {points = 0, time = 0}
            science_packs[ingredient].points = science_packs[ingredient].points + data.points
            science_packs[ingredient].time = science_packs[ingredient].time + data.time
        end
        ...
        for ingredient, data in pairs(science_packs) do
            local ingredient_inputs = venv.input.items[ingredient]
            if ingredient_inputs then
                -- calculating input science pack count in normal quality equivalent
                -- i.e. uncommon counts as 2 packs, rare as 3, etc.
                local total_count = 0
                for quality, count in pairs(ingredient_inputs) do
                    local quality_level = prototypes.quality[quality].level
                    total_count = total_count + count * quality_level
                end

                local ppi = data.points / total_count
                template_data.science.points_per_item = template_data.science.points_per_item or {}
                template_data.science.points_per_item[ingredient] = ppi
            else
    ...
    else -- surface type 1 meaning production template
...
function Helper.start_compilation(surface_id, template_name)
    local check = Helper.can_start_compilation(surface_id, template_name)
    if not check then return end

    local vsurface_data = storage.v_surfaces[surface_id]
    local surface = game.get_surface(surface_id)
    -- type 2 is science surface
    if vsurface_data.type == 2 then
    ...
    storage.compiling_surfaces[surface_id] = {
        ...
        surface_type = vsurface_data.type,
    ...
    -- if surface type is science we need to allocate time for benchmarking each science pack
    ...
    chunk_registry.switch_lab_state(surface_id, "compiling")
    return true
end

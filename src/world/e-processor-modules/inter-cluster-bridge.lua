...
I. Mandatory entity controls are provided:
    1. first_cluster. Used to determine the source cluster from which
        items are taken.
    2. second_cluster. Used to determine the destination cluster to which
        items are transfered.
    3. operation_mode (item/fluid/energy). Used to determine the buffer
        entry this entity will attempt to reach.
    4. seleted_item (only for "item" mode). Used to determine the buffer
        entry this entity will attempt to reach.
    5. selected_fluid (only for "fluid" mode). Used to determine the buffer
        entry this entity will attempt to reach.
...
function ClusterBridge.initialize(properties)
    ...
    if not source_uuid then
        properties.status = Utilities.entity_status.no_source_cluster
    ...
    if not destination_uuid then
        properties.status = Utilities.entity_status.no_destination_cluster
        return Utilities.registry_sections.incorrect
    end
...
function ClusterBridge.update(properties)
    ...
    -- Checking if buffer entry exists in source cluster
    ...
    -- Checking if buffer entry exists in destination cluster
    ---@type string checked on initialization
    local destination_uuid = properties.second_cluster
